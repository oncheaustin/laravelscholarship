...
@section('content')
    ...
    <div class="form-group">
                    ...
                    <div class="col-md-8 col-md-offset-2">
                        <input type="hidden" name="charge" id="charge" value="1 NGN = {{ $fund->payment->rate }}" class="form-control input-lg" required readonly hidden placeholder="">
                    </div>
                </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    <div class="input-group">
                                        ...
                                        <span class="input-group-addon red">&nbsp;<strong> NGN </strong></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="form-group">
                                        <div class="col-sm-6 col-sm-offset-3">
                                            <button class="btn btn-primary btn-block btn-icon bold icon-left"><i
                                                        class="fa fa-send"></i> Make Payment Now</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                        ...
                        @elseif($fund->payment_type == 4)
                            <div class="row">
                               ...
                               <!-- {!! Form::open() !!}-->
                                ...
                                <input type="hidden" name="fund_id" value="{{ $fund->id }}">
                                <input type="hidden" id="custom" name="custom" value="{{ $fund->custom }}">
                                <input type="hidden"id="email" name="email" value="{{ $email->email }}">
                                <input type="hidden"id="name" name="name" value="{{ $email->name }}">
                                <input type="hidden"id="phone" name="phone" value="{{ $email->phone }}">
                                <input type="hidden" name="url" value="{{ route('deposit-fund') }}">
                                <div class="form-group">
                                    <div class="col-sm-6 col-sm-offset-3">
                                        <button onclick="payWithPaystack1()" class="btn btn-primary btn-block bold btn-icon icon-left"><i
                                                    class="fa fa-send"></i>Make Payment Now</button>
                                    </div>
                                </div>
                               <!-- {{ Form::close() }} -->
                    ...
                    @else
                        ...
                        <div class="row">
                            ...
                            <div class="form-group">
                                   <div class="text-center">
                                        <h6 class="bold" style="font-size: 24px;border-bottom: 2px dotted ;padding-bottom: 15px">After Payment please call/text support +2348160295093 for quick confirmation.</h6>
                                    </div>
...
<script>
  function payWithPaystack1(){
    ...
    var handler = PaystackPop.setup({
      key: 'pk_live_e62e2e26ca5dd71d3650a98dc5c8a44676dd0796',
      email: email,
      amount: {{ round(($fund->usd *100),2)  }},
      ref: custom, // generates a pseudo-unique reference. Please replace with a reference you generated. Or remove the line entirely so our API will generate one for you
      metadata: {
         custom_fields: [
            {
                display_name: name,
      ...
      callback: function(response){
          window.location = "https://tomalanescholarship.com/user/deposit-verify/?reference=" + response.reference;
          
             //  alert('Payment Completed. transaction ref is ' + response.reference);
      },
      onClose: function(){
          alert('You want to Stop payment?');
          
      }
      
      /*
          // post to server to verify transaction before giving value
        var verifying = $.get( '/user/deposit-preview/?reference=' + response.reference);
        verifying.done(function( data ) { //give value });
        
          alert('success. transaction ref is ' + response.reference);
          header("Location: " . 'agromonth.com/user');
      },
      onClose: function(){
          alert('window closed');
      }
