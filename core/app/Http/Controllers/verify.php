...
//        $data['bank'] = PaymentMethod::orderBy('id','asc')->skip($skip)->take($limit)->whereStatus(1)->get();
    ...
    {
        $data['page_title'] = 'Payment Verification';
...
'Authorization: Bearer sk_live_648fdb96a58975e9c98fcbdbce0fd2d5b047bdf4']
);
...
$basic = BasicSetting::first();
 ...
 $mem = User::findOrFail($mem->id);
 


 
 $custom = $_GET['reference'];
 $data = PaymentLog::select('custom','amount','charge','net_amount')->where('custom', $custom)->first();
